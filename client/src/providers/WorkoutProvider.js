...
const WorkoutProvider = ({ children }) => {
  ...
  const [newestWorkouts, setNewestWorkouts] = useState([]);
  ...
  const getWorkouts = (page = 1) => {
    axios
      .get(`/api/workouts?page=${page}`)
      .then((res) => {
        const { headers, data } = res;
        const totalPages = Math.ceil(
          parseInt(headers["total"]) / parseInt(headers["per-page"])
        );
  ...
  const getPopularWorkouts = () => {
    axios
      .get("/api/popularWorkouts")
  ...
  const getNewestWorkouts = () => {
    axios
      .get("/api/newestWorkouts")
      .then((res) => {
